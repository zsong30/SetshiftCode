function lfpdata = do_NP_LFP_50ms_predictionmodel_csf02
...
%                     figpath = 'Z:\projmon\ericsprojects\Setshift\ANALYSES\representative analyses\lfp-behavior-correlation\CSF02\CSF02_PL_09232021';
                ...
                for bl=1:length(setshift.rules(rl).blocks)
                    for tl = 1:length(setshift.rules(rl).blocks(bl).trials)
                        ...
                        elseif setshift.rules(rl).blocks(bl).trials(tl).performance == 0
                            ...
                            incorrectInitiationT = [correctInitiationT,setshift.rules(rl).blocks(bl).trials(tl).initiation_time];
                        end
                    end
                end
            end

            correctDMSegments = [round(correctInitiationT*2500);round(correctResponseT*2500)]'; % Splitting this early was wrong, need to have DM and learning together going into cleaning and all that. Separate after.
            correctLearningSegments = [round(correctResponseT*2500);round(correctResponseT*2500)+2500]';
            cleandatacorrectDM = get_clean_NP_data_automatic_ES(trimmedData,RawData.Timestamps,correctDMSegments);
            cleandatacorrectLearning = get_clean_NP_data_automatic_ES(trimmedData, RawData.Timestamps, correctLearningSegments);
            ...
            % segment.
            correctDMdata = nan(size(cleandatacorrectDM.trial,2),size(cleandatacorrectDM.label,1),size(cleandatacorrectDM.trial{1,1},2)); 
            for i = 1:size(cleandatacorrectDM.trial,2)
                correctDMdata(i,:,:) = cleandatacorrectDM.trial{1,i};
            end
            correctDMdata = permute(correctDMdata,[3,2,1]);% now sample x chan x trial
            day(dy).correctDMdata = correctDMdata; 
            clear correctDMdata;
            correctLearningdata = nan(size(cleandatacorrectLearning.trial,2),size(cleandatacorrectLearning.label,1),size(cleandatacorrectLearning.trial{1,1},2)); 
            for i = 1:size(cleandatacorrectLearning.trial,2)
                correctLearningdata(i,:,:) = cleandatacorrectLearning.trial{1,i};
            end
            correctLearningdata = permute(correctLearningdata,[3,2,1]);% now sample x chan x trial
            day(dy).correctLearningdata = correctLearningdata; 
            clear correctLearningdata;

            incorrectDMSegments = [round(incorrectInitiationT*2500);round(incorrectResponseT*2500)]';
            incorrectLearningSegments = [round(incorrectResponseT*2500);round(incorrectResponseT*2500)+2500]';
            cleandataincorrectDM = get_clean_NP_data_automatic_ES(trimmedData,RawData.Timestamps,incorrectDMSegments);
            cleandataincorrectLearning = get_clean_NP_data_automatic_ES(trimmedData, RawData.Timestamps, incorrectLearningSegments);
            incorrectDMdata = nan(size(cleandataincorrectDM.trial,2),size(cleandataincorrectDM.label,1),size(cleandataincorrectDM.trial{1,1},2)); 
            for i = 1:size(cleandataincorrectDM.trial,2)
                incorrectDMdata(i,:,:) = incleandataincorrectDM.trial{1,i};
            end
            incorrectDMdata = permute(incorrectDMdata,[3,2,1]);% now sample x chan x trial
            day(dy).incorrectDMdata = incorrectDMdata; 
            clear incorrectDMdata;
            incorrectLearningdata = nan(size(cleandataincorrectLearning.trial,2),size(cleandataincorrectLearning.label,1),size(cleandataincorrectLearning.trial{1,1},2)); 
            for i = 1:size(cleandataincorrectLearning.trial,2)
                incorrectLearningdata(i,:,:) = cleandataincorrectLearning.trial{1,i};
            end
            incorrectLearningdata = permute(incorrectLearningdata,[3,2,1]);% now sample x chan x trial
            day(dy).incorrectLearningdata = incorrectLearningdata; 
            clear incorrectLearningdata;
