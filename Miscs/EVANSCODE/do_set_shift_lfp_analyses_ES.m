...
minsec = 3; % minimal length of data for analyses
...
region(2).name = 'dorsalST';% for striatum 
%region(3).name = 'ventralST';% for striatum 
...
for rt = 1:length(rat)
    for dy = 1:length(rat(rt).day)
        ...
        % Evan: moved data loading and downsampling outside comparison loop
        RawData1=load_open_ephys_binary(rat(rt).day(dy).epath, 'continuous',1,'mmap');
        disp("Data loaded")
        % Evan: no trimming, use all channels
%             if rt == 1
%                 trimedData = double(RawData1.Data.Data(1).mapped([V2Dorder(12:15),V2Dorder2(2:3),V2Dorder2(30:31)],:));
%             elseif rt == 2
%                 trimedData = double(RawData1.Data.Data(1).mapped([V2Dorder(28:31),V2Dorder2(2:3),V2Dorder2(30:31)],:));
%             end
        ...
        % Evan: changed to loop to accomadate for larger matrix
        trimedData=zeros(size(RawData1.Data.Data(1).mapped,1),floor((size(RawData1.Data.Data(1).mapped,2)-1)/20+1));
        for i=1:size(trimedData,1)
            trimedData(i,:)=downsample(RawData1.Data.Data(1).mapped(i,:),20); % downsampling; nChan x nSamples
        end
        disp("Data downsampled")
        RawData1.Timestamps=double(downsample(RawData1.Timestamps,20));% downsampling timestamps;
        
        for comparison = 1:7 % see below for each comparison
            ...
            elseif comparison == 7 % Error type comparison
                condition(2*comparison-1).name = 'Perseverative'; condition(2*comparison).name = 'Post-Perseverative'; condition(2*comparison+1).name = 'Regressive';
                for seg = 1:3
                    temp = ...
                        [temp;trialsegments(seg).tasktimes_perseverative];
                    temp2 = ...
                        [temp2;trialsegments(seg).tasktimes_post_perseverative];
                    temp3 = ...
                        [temp3;trialsegments(seg).tasktimes_regressive];
                end
                condition(2*comparison-1).segment = temp;
                condition(2*comparison).segment = temp2;
                condition(2*comparison+1).segment = temp3;
            end  
...
%                 %%% THE ABOVE LINE NEEDS TO BE CHANGED TO THE FOLLOWING
%                 %%% SStimestamps = double(TSdata.Timestamps)-RawData1.Timestamps(1); 
            end
            ...
            for i = 1:size(condition(2*comparison-1).segment,1)
                index_trial_starts(i)=find(RawData1.Timestamps>condition(2*comparison-1).segment(i,1),1,'first'); %#ok<*AGROW>
                index_trial_resps(i)=find(RawData1.Timestamps>condition(2*comparison-1).segment(i,2),1,'first'); %#ok<*AGROW>
            end
            segments_starts = [index_trial_starts-Frq*minsec/2;index_trial_starts+Frq*minsec/2]'; % time indice in samples
            segments_resps = [index_trial_resps-Frq*minsec/2;index_trial_resps+Frq*minsec/2]'; % time indice in samples
            % data = [region(1).data;region(2).data]; % combine for artifact rejection
            % clean data and reject artifact
            data = trimedData;
            clean_segmented_data_starts = get_clean_data_automatic_ES(data,RawData1.Timestamps,segments_starts);
            clean_segmented_data_resps = get_clean_data_automatic_ES(data,RawData1.Timestamps,segments_resps);
            
            % Evan: added option for Laplacian or no rereferencing
            if strcmp(reref,'Laplacian')
                for i = 1:size(clean_segmented_data_starts.trial,2)
                    % Evan: iterate over all channels
                    cleanMat=clean_segmented_data_starts.trial{1,i};
                    for j=2:length(V2Dorder)-1
                        region(1).condition(2*comparison-1).data_starts(i,:,j-1) = cleanMat(V2Dorder(j),:)-(cleanMat(V2Dorder(j-1),:)+cleanMat(V2Dorder(j+1),:))/2;
                    end

                    for j=2:length(V2Dorder2)-1
                        region(2).condition(2*comparison-1).data_starts(i,:,j-1) = cleanMat(V2Dorder2(j),:)-(cleanMat(V2Dorder2(j-1),:)+cleanMat(V2Dorder2(j+1),:))/2;
                    end
                end
                for i = 1:size(clean_segmented_data_resps.trial,2)
                    % Evan: iterate over all channels
                    cleanMat=clean_segmented_data_resps.trial{1,i};
                    for j=2:length(V2Dorder)-1
                        region(1).condition(2*comparison-1).data_resps(i,:,j-1) = cleanMat(V2Dorder(j),:)-(cleanMat(V2Dorder(j-1),:)+cleanMat(V2Dorder(j+1),:))/2;
                    end

                    for j=2:length(V2Dorder2)-1
                        region(2).condition(2*comparison-1).data_resps(i,:,j-1) = cleanMat(V2Dorder2(j),:)-(cleanMat(V2Dorder2(j-1),:)+cleanMat(V2Dorder2(j+1),:))/2;
                    end
                end
            else
                for i = 1:size(clean_segmented_data_starts.trial,2)
                    % Evan: iterate over all channels
                    cleanMat=clean_segmented_data_starts.trial{1,i};
                    for j=1:length(V2Dorder)
                        region(1).condition(2*comparison-1).data_starts(i,:,j) = cleanMat(V2Dorder(j),:);
                    end

                    for j=1:length(V2Dorder2)
                        region(2).condition(2*comparison-1).data_starts(i,:,j) = cleanMat(V2Dorder2(j),:);
                    end
                end
                for i = 1:size(clean_segmented_data_resps.trial,2)
                    % Evan: iterate over all channels
                    cleanMat=clean_segmented_data_resps.trial{1,i};
                    for j=1:length(V2Dorder)
                        region(1).condition(2*comparison-1).data_resps(i,:,j) = cleanMat(V2Dorder(j),:);
                    end

                    for j=1:length(V2Dorder2)
                        region(2).condition(2*comparison-1).data_resps(i,:,j) = cleanMat(V2Dorder2(j),:);
                    end
                end
            end
            ...
            clear index_trial_starts index_trial_resps segments_starts segments_resps
            
            % find index of the trial segments in lfp times for condition 2
            % find index of the trial segments in lfp times
            for i = 1:size(condition(2*comparison).segment,1)
                index_trial_starts(i)=find(RawData1.Timestamps>condition(2*comparison).segment(i,1),1,'first'); %#ok<*AGROW>
                index_trial_resps(i)=find(RawData1.Timestamps>condition(2*comparison).segment(i,2),1,'first'); %#ok<*AGROW>
            end
            % Evan: implemented distinct logic for interval epoching,
            % trials are 2s long and response is empty
            if comparison ~= 1
                segments_starts = [index_trial_starts-Frq*minsec/2;index_trial_starts+Frq*minsec/2]'; % time indice in samples
                segments_resps = [index_trial_resps-Frq*minsec/2;index_trial_resps+Frq*minsec/2]'; % time indice in samples
                % clean data and reject artifact
                ...
                clean_segmented_data_resps = get_clean_data_automatic_ES(data,RawData1.Timestamps,segments_resps);
                 ...
                 if strcmp(reref,'Laplacian')
                    for i = 1:size(clean_segmented_data_starts.trial,2)
                        ...
                        cleanMat=clean_segmented_data_starts.trial{1,i};
                        for j=2:length(V2Dorder)-1
                            region(1).condition(2*comparison).data_starts(i,:,j-1) = cleanMat(V2Dorder(j),:)-(cleanMat(V2Dorder(j-1),:)+cleanMat(V2Dorder(j+1),:))/2;
                        end

                        for j=2:length(V2Dorder2)-1
                            region(2).condition(2*comparison).data_starts(i,:,j-1) = cleanMat(V2Dorder2(j),:)-(cleanMat(V2Dorder2(j-1),:)+cleanMat(V2Dorder2(j+1),:))/2;
                        end
                    end
                    for i = 1:size(clean_segmented_data_resps.trial,2)
                        % Evan: iterate over all channels
                        cleanMat=clean_segmented_data_resps.trial{1,i};
                        for j=2:length(V2Dorder)-1
                            region(1).condition(2*comparison).data_resps(i,:,j-1) = cleanMat(V2Dorder(j),:)-(cleanMat(V2Dorder(j-1),:)+cleanMat(V2Dorder(j+1),:))/2;
                        end

                        for j=2:length(V2Dorder2)-1
                            region(2).condition(2*comparison).data_resps(i,:,j-1) = cleanMat(V2Dorder2(j),:)-(cleanMat(V2Dorder2(j-1),:)+cleanMat(V2Dorder2(j+1),:))/2;
                        end
                    end
                else
                    for i = 1:size(clean_segmented_data_starts.trial,2)
                        ...
                        cleanMat=clean_segmented_data_starts.trial{1,i};
                        for j=1:length(V2Dorder)
                            region(1).condition(2*comparison).data_starts(i,:,j) = cleanMat(V2Dorder(j),:);
                        end

                        for j=1:length(V2Dorder2)
                            region(2).condition(2*comparison).data_starts(i,:,j) = cleanMat(V2Dorder2(j),:);
                        end
                    end
                    for i = 1:size(clean_segmented_data_resps.trial,2)
                        % Evan: iterate over all channels
                        cleanMat=clean_segmented_data_resps.trial{1,i};
                        for j=1:length(V2Dorder)
                            region(1).condition(2*comparison).data_resps(i,:,j) = cleanMat(V2Dorder(j),:);
                        end

                        for j=1:length(V2Dorder2)
                            region(2).condition(2*comparison).data_resps(i,:,j) = cleanMat(V2Dorder2(j),:);
                        end
                    end
                end
            else
                segments_starts = [index_trial_starts;index_trial_starts+Frq*2]';
                % clean data and reject artifact
                clean_segmented_data_starts = get_clean_data_automatic_ES(data,RawData1.Timestamps,segments_starts);
                % Evan: added option for Laplacian or no rereferencing
                if strcmp(reref,'Laplacian')
                ...
                region(2).condition(2*comparison).data_resps=[];
            end
            for reg = 1:2
            ...
            if comparison == 7
                % find index of the trial segments in lfp times for
                % condition 3 (if condition 7)
                for i = 1:size(condition(2*comparison+1).segment,1)
                    index_trial_starts(i)=find(RawData1.Timestamps>condition(2*comparison+1).segment(i,1),1,'first'); %#ok<*AGROW>
                    index_trial_resps(i)=find(RawData1.Timestamps>condition(2*comparison+1).segment(i,2),1,'first'); %#ok<*AGROW>
                end
        ...
        disp(strcat("Comparison ", num2str(comparison), " done"))
        clear region
    end % end of day
end % end of rat
