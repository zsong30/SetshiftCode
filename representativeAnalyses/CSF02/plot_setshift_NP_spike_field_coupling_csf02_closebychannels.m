    function plot_setshift_NP_spike_field_coupling_csf02_closebychannels
...
for i = 6:length(frq)-1
    for u = 1:length(unit2plot)
        fig = figure('name',sprintf('%s-%s-%s-frqband %d-%d_unit%d',ratname,datadate,regionname,frq(i).band(1),frq(i).band(2),unit2plot(u)));
    ...
    allunitphase = [];
        ...
        unit_ch = trial.units(unit2plot(u)).channel;
channels2include = unit_ch-5:unit_ch+5;
    ...
    figname1 = ...
        sprintf('%s-%s-%s-frqband %d-%d %d units ISI %d-%d %s unit%d_closebychannels',ratname,datadate,regionname,frq(i).band(1),frq(i).band(2),length(unit2plot),ISIsLowerLimit,ISIsUpLimit,plotname,unit2plot(u));
