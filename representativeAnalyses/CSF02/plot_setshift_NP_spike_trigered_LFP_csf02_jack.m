 function plot_setshift_NP_spike_trigered_LFP_csf02_jack
...
figpath = 'Z:\projmon\ericsprojects\Setshift\ANALYSES\representative analyses\spike-field-coupling\plot_setshift_NP_spike_trigered_LFP\CSF02\CSF02_PL_09232021_all_traces_from_unit';
...
ISIsLowerLimit = ISIsLimits(1);
    ...
    lfp_10_units = [];
    % fig = figure('name',sprintf('%s-%s-%s-all_traces_spike_triggered_LFP_200_ms_window_units_%d_and_%d',ratname,datadate,regionname,unit2plot(1),unit2plot(2)));
    fig = figure('name',sprintf('%s-%s-%s-all_traces_spike_triggered_LFP_2_second_window_units_%d_and_%d',ratname,datadate,regionname,unit2plot(1),unit2plot(2)));
    ...
    for u = 1:length(unit2plot)
        ...
        if sum(trial.units(unit2plot(u)).ISIs<ISIsUpLimit & trial.units(unit2plot(u)).ISIs>ISIsLowerLimit) > 0
            ...
            % for k = 1 % 1 raw trace
                % if tmp_st(k)-1250 >= 0 && tmp_st(k)+1250 <= tmp_st(end) % 500 ms before and after spike
                if tmp_st(k)-2500 >= 0 && tmp_st(k)+2500 <= tmp_st(end) % 1s before and after spike
                % if tmp_st(k)- 250 >= 0 && tmp_st(k)+250 <= tmp_st(end) % 100 ms before and after spike
                    % tmp_lfp(k,:) =
                    % lfp(tmp_st(k)-25:tmp_st(k)+25)-median(lfp(tmp_st(k)-25:tmp_st(k)-20)); % 10 ms before and after spike 
                    % tmp_lfp(k,:) = lfp(tmp_st(k)-1250:tmp_st(k)+1250)- median(lfp(tmp_st(k)-1250:tmp_st(k)-1000)); % 500 ms before and after spike
                    tmp_lfp(k,:) = lfp(tmp_st(k)-2500:tmp_st(k)+2500)- median(lfp(tmp_st(k)-2500:tmp_st(k)-2495)); % 1s before and after spike
                    % tmp_lfp(k,:) = lfp(tmp_st(k)-250:tmp_st(k)+250)- median(lfp(tmp_st(k)-250:tmp_st(k)-245)); % 100 ms before and after spike
                else 
                end
            end
            ...
            t = linspace(-1000,1000,5001);
            % t = linspace(-100,100,501);
            % lfp_mean = mean(tmp_lfp,1,'omitnan');
            plot(t,mean(tmp_lfp,1,'omitnan'));
            xlabel('ms')
            ylabel('uV')
    ...
    % fig2 = figure('name',sprintf('%s-%s-%s-spike_triggered_LFP_allunits_2_sec_window',ratname,datadate,regionname));
    % 
    % 
    %         % plot((-20:20/25:20),mean(allunitlfp,1));
    %         % plot((-1000:1000/1250:1000),mean(allunitlfp,1));
    %         t = linspace(-1000,1000,5001);
    %         plot(t,mean(allunitlfp,1));
    %         xlabel('ms')
    %         ylabel('uV')
    %         % ylim([-85 40])
    % 
    ...
    % %     unit(i)data = tmp_lfp(i,:);
    % %     % SA(unit(i)data);
    % % end

lfp_11_units;    
datapath = 'Z:\projmon\ericsprojects\Setshift\EPHYSDATA\NP\CSF02\2021-09-23_14-50-18';
save(sprintf('%s-%s-%s-lfp_first_10_units_avg',ratname,datadate,regionname),"lfp_11_units");
toc