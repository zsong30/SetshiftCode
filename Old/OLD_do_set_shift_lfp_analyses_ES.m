...
%                 %%% SStimestamps = double(TSdata.Timestamps)-RawData1.Timestamps(1); 
            ...
            for i = 1:size(clean_segmented_data.trial,2)
                ...
                region(3).condition(2*comparison-1).data(i,:) = clean_segmented_data.trial{1,i}(8,:)-clean_segmented_data.trial{1,i}(7,:);
            end
            for reg = 1:3
            ...
            segments = [index_tri al;index_trial+Frq*minsec]';
            % clean data and reject artifact
            clean_segmented_data = get_clean_data_automatic_ES(data,RawData1.Timestamps,segments);
            % biplor referencing
            for i = 1:size(clean_segmented_data.trial,2)
                ...
                region(3).condition(2*comparison).data(i,:) = clean_segmented_data.trial{1,i}(8,:)-clean_segmented_data.trial{1,i}(7,:);
            end
            for reg = 1:3
