function stats = plot_set_shift_2x32ch_lfp12112021(rat)
...
Rat(1).Prelimbic.subregion(1).channels = [2,3,4]; % D2V order, the way data is organized.
Rat(1).Prelimbic.subregion(2).channels = [29,30,31]; % D2V order, the way data is organized.
Rat(1).Striatum.subregion(1).channels = [2,3,4]+32;
Rat(1).Striatum.subregion(2).channels = [29,30,31]+32;

Rat(2).Prelimbic.subregion(1).channels = [2,3,4];
Rat(2).Prelimbic.subregion(2).channels = [29,30,31];
Rat(2).Striatum.subregion(1).channels = [2,3,4]+32;
Rat(2).Striatum.subregion(2).channels = [29,30,31]+32;
            ...
            for bl = 1:length(rat(rt).setshift(s).rules(rl).blocks)
                ...
                for tr = 1:length(rat(rt).setshift(s).rules(rl).blocks(bl).trials)
                    ...
                    if isfield(rat(rt).setshift(s).rules(rl).blocks(bl).trials(tr),'region') ...
                        ...
                        baseline_lfp = ... % 64 chan x timestamps x trials
...
for comp = 1:4 % loop in all comparisons; 6 is hard coded, but can be any number.
    ...
    % 64 chan x timestamps x trial for r_task_lfp
    if comp == 1
        data1 = squeeze(Rat(rt).baseline_lfp(Rat(rt).Prelimbic.subregion(reg).channels(ch),:,:) - median(Rat(rt).baseline_lfp((1:32),:,:),1));
        data1 = data1(:,comparison(comp).condition(1).index);
        
        data2 = squeeze(Rat(rt).baseline_lfp(Rat(rt).Striatum.subregion(reg).channels(ch),:,:) - median(Rat(rt).baseline_lfp((1:32),:,:),1));
        data2 = data2(:,comparison(comp).condition(1).index);
                
        data3 = squeeze(Rat(rt).task_lfp(Rat(rt).Prelimbic.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((1:32),:,:),1));
        data3 = data3(:,comparison(comp).condition(2).index);
        
        data4 = squeeze(Rat(rt).task_lfp(Rat(rt).Striatum.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((1:32),:,:),1));
        data4 = data4(:,comparison(comp).condition(2).index);

    else
        
        data1 = squeeze(Rat(rt).task_lfp(Rat(rt).Prelimbic.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((1:32),:,:),1));
        data1 = data1(:,comparison(comp).condition(1).index);
        
        data2 = squeeze(Rat(rt).task_lfp(Rat(rt).Striatum.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((1:32),:,:),1));
        data2 = data2(:,comparison(comp).condition(1).index);
    ...
    reg = 2; %  ventral subregions for both PL and ST
     ...
     if comp == 1
        data5 = squeeze(Rat(rt).baseline_lfp(Rat(rt).Prelimbic.subregion(reg).channels(ch),:,:) - median(Rat(rt).baseline_lfp((33:64),:,:),1));
        data5 = data5(:,comparison(comp).condition(1).index);
        
        data6 = squeeze(Rat(rt).baseline_lfp(Rat(rt).Striatum.subregion(reg).channels(ch),:,:) - median(Rat(rt).baseline_lfp((33:64),:,:),1));
        data6 = data6(:,comparison(comp).condition(1).index);
        
        
        data7 = squeeze(Rat(rt).task_lfp(Rat(rt).Prelimbic.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((33:64),:,:),1));
        data7 = data7(:,comparison(comp).condition(2).index);
        
        data8 = squeeze(Rat(rt).task_lfp(Rat(rt).Striatum.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((33:64),:,:),1));
        data8 = data8(:,comparison(comp).condition(2).index);

    else
        
        data5 = squeeze(Rat(rt).task_lfp(Rat(rt).Prelimbic.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((33:64),:,:),1));
        data5 = data5(:,comparison(comp).condition(1).index);
        
        data6 = squeeze(Rat(rt).task_lfp(Rat(rt).Striatum.subregion(reg).channels(ch),:,:) - median(Rat(rt).task_lfp((33:64),:,:),1));
        data6 = data6(:,comparison(comp).condition(1).index);
    ...
    for ch = 1:length(channel)
       ...
       S1_1 = cat(3,channel(ch).S1_1,S1_1);
    ...
    Mn_S2_1= median(S2_1,2,'omitnan')/NormFactor;
    SEM_S2_1= std(S2_1,0,2,'omitnan')./sqrt(size(data1,1))/NormFactor;
    
    Mn_C_1= median(C_1,2,'omitnan');
