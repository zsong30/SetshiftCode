...
% plot_set_shift_lfp_analyses(rat,2)
...
%     [C_2b,phib,S12_2b,S1_2b,S2_2b,tb,fb]=cohgramc(data3,data4,movingwin,params); %#ok<ASGLU>
...
%     SEM_C_1b=std(squeeze(mean(C_1b,1)),0,2)./sqrt(size(data1,1));
    ...
    %hp2b=plot(f,Mn_S2_1b','LineWidth',3,'Color','y','LineStyle',':');
...
%     myleg = {region(1).name,region(2).name,region(2).name};
%     legend([hp1,hp2,hp2b], myleg);
    ...
    %     p2 = plot(f,Mn_C_1b','LineWidth',3,'Color','y','LineStyle',':');
    %     patch([f,fliplr(f)], [(Mn_C_1b-SEM_C_1b)',flipud(Mn_C_1b+SEM_C_1b)'],'y','EdgeColor','none', 'FaceAlpha',0.2)
    ...
    %     hp2b=plot(f,Mn_S2_2b','LineWidth',3,'Color','y','LineStyle',':');
    ...
    %     patch([f,fliplr(f)], [(Mn_S2_2b-SEM_S2_2b)',flipud(Mn_S2_2b+SEM_S2_2b)'],'y','EdgeColor','none', 'FaceAlpha',0.2);
    myleg = {region(1).name,region(2).name};
    legend([hp1,hp2], myleg);
    ...
    figpath = 'G:\My Drive\Setshift\figures';
