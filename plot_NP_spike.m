function plot_NP_spike_triggered_lfp_power
...
ISIsLowerLimit = ISIsLimits(1);
    ...
    % fig = figure('name',sprintf('%s-%s-%s-non_phase_locked_spike_triggered_LFP_2_second_window_unit_10_(2)',ratname,datadate,regionname));
     % fig = figure('name',sprintf('%s-%s-%s-phase_locked_traces_spike_triggered_LFP_3_traces_200_ms_window_unit_74',ratname,datadate,regionname));
     fig = figure('name',sprintf('%s-%s-%s-phase_locked_traces_spike_triggered_LFP_3_traces_200_ms_window_unit_%d',ratname,datadate,regionname,unit2plot));
    ...
    for u = 1:length(unit2plot)
        ...
        for j = 1:size(tmp_lfp,1)
            ...
            if phase(j,250) >= -75 && phase(j,250) <= -65 % range for average phase locking for unit 74
                ...
                phase_lock_spikes(j,:) = nan(1,501);
            end
        end
        phase_lock_spikes_values = phase_lock_spikes(~isnan(phase_lock_spikes(:,1)),:);
