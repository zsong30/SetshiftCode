function plot_NP_spike_triggered_lfp_power_and_power
...
unit2plot = 76; %phase locked
% unit2plot = 75; %phase locked
...
ISIsLowerLimit = ISIsLimits(1);
    ...
    for u = 1:length(unit2plot)
        ...
        for j = 1:size(tmp_lfp,1)
            ...
            if phase(j,250) >= -161 && phase(j,250) <= -151 % range for average phase locking for unit 76
            % if phase(j,250) >= -88 && phase(j,250) <= -78 % range for average phase locking for unit 75
        ...
        phase_lock_spikes_chosen = phase_lock_spikes_values(~isnan(phase_lock_spikes_values(:,1)),:);
          ...
          % for i = 1:length(phase_lock_spikes')
            ...
            fig2 = figure('name',sprintf('%s-%s-%s-spike_triggered_LFP_phase_locked_traces_avg_200_ms_window_unit_%d',ratname,datadate,regionname,unit2plot));
